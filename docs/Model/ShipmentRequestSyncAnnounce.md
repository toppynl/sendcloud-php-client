...
**parcels** | [**\Toppy\Sendcloud\Model\ParcelCommon[]**](ParcelCommon.md) | Represent each package of the shipment. Currently it&#39;s only allowed to send max 15 parcels per shipment in this endpoint. In order to announce more than 15 parcels in the same shipment, please refer to the **[Create and announce a shipment asynchronously](https://api.sendcloud.dev/docs/sendcloud-public-api/branches/v3/shipments/operations/create-a-shipment) endpoint**. Note: each carrier can have a lower than the default number of parcels limit per shipment. | [optional]
